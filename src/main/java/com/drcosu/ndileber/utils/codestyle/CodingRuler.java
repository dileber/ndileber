...
package com.drcosu.ndileber.utils.codestyle;
 ...
 * <li>1.2. <strong color='red'>去掉无用的包、方法、变量等，减少僵尸代码。</strong>
 * <li>1.3. 使用 Lint 工具来查看并消除警告和错误。
 * <li>1.4. 使用 Ctrl+Shift+F 来格式化代码，然后再进行调整。
 * <li>1.5. 使用 Ctrl+Shift+O 来格式化 Import 包。
 * <li>1.6. 类文件内,<strong color='red'>首行代码之前务必使用换行将类定义分开</strong></li>
 * <li>1.7. 适当的换行将代码按功能分块</li>
 * </ul>
 * <p>
 * <h3>2. 命名规则</h3>
 * <h3>2.1. 基本原则</h3>
 ...
 * <li>2.1.2. 命名不能太长，适当使用简写或缩写。（最好不要超过 25 个字母）【以准确为主】
 * <li>2.1.3. 方法名以小写字母开始，以后每个单词首字母大写。
 * <li>2.1.4. 避免使用相似或者仅在大小写上有区别的名字。
 * <li>2.1.5. 避免使用数字，但可用 2 代替 to，用 4 代替 for 等，如 go2Clean。
 * <li>2.1.6. 避免出现拼写错误 eg: provice(省份) -> province。
 * </ul>
 * <p>
 * <h3>2.2. 类、接口</h3>
 * <ul>
 * <li>2.2.1. 所有单词首字母都大写。使用能确切反应该类、接口含义、功能等的词。一般采用名词。
 * <li>2.2.2. 接口带 I 前缀，或able, ible, er等后缀。如ISeriable。【可选,<strong color='red'>避免编码</strong>】
 * </ul>
 * <p>
 * <h3>2.3. 字段、常量</h3>
 * <ul>
 * <li>2.3.1. 成员变量以 m 开头，静态变量以 s 开头，如 mUserName, sInstance。【可选,<strong color='red'>避免编码</strong>】
 ...
 * <h3>2.4. 资源</h3>
 * <ul>
 * <li>2.4.1. values下资源文件内资源定义全部字母小写,词与词之间使用 _ 分隔</li>
 * </ul>
 * </p>
 * <p>
 * <h3>3. 注释</h3>
 ...
 * <li>3.4. 尽量让代码自说明或者注释准确
 * <li>3.5. 如果使用注释,在升级代码时,同时升级注释
 * </ul>
 * <p>
 ...
 * 从而达到良好的可读性。
 * <li>4.2. 总体上来说，要按照先 public, 后 protected, 最后 private, 函数的排布<strong color='red'>遵循向下规则。</strong>
 * <li>4.3. 以下顺序可供参考：<br>
 * 定义TAG，一般为 private,一般使用类名（可选）<br>
 * 定义 public 常量<br>
 ...
 * <p>
 * <h3> 6. 资源定义</h3>
 * <h3>6.1. 类内部避免使用中文,将使用到的内容定义在strings.xml内部</h3>
 *
 */
