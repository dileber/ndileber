...
import com.drcosu.ndileber.tools.file.AttachmentStore;
...
public class ImageUtil {
	...
	public static Bitmap getDefaultBitmapWhenGetFail() {
        try {
            return getBitmapImmutableCopy(SApplication.getAppContext().getResources(), R.mipmap.dileber_image_download_failed);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
     ...
     * @param srcFile 缩放源文件
     * @param dstFile 缩放文件
     * @param dstMaxWH 缩放最大高宽
     * @param dstMinWH 缩放最小高宽
    ...
    private static String getTempFilePath(String extension) {
    	return UStorage.getWritePath(
    			"temp_image_" + HString.get36UUID() + "." + extension,
    ...
    public static int[] getBoundWithLength(int maxSide, Object imageObject, boolean resizeToDefault) {
        ...
        if (String.class.isInstance(imageObject)) {
        	...
        	bound = BitmapDecoder.decodeBound(SApplication.getAppContext().getResources(), (Integer) imageObject);
        	width = bound[0];
        	height = bound[1];
    ...
    public static Bitmap getBitmapFromDrawableRes(int res) {
        try {
            return getBitmapImmutableCopy(SApplication.getAppContext().getResources(), res);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
